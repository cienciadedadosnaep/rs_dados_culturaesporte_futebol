...
tabela <- brasileirao_serie_a %>% select(ano_campeonato,estadio,gols_man,gols_vis) %>% 
  filter(estadio == "Arena Fonte Nova") %>% filter(ano_campeonato >= 2004)

tabela2 <- tabela %>% group_by(ano_campeonato) %>% select(gols_man,gols_vis) %>%
  summarise(sum(gols_man,gols_vis)) 

names(tabela2) <- c("ano","Gols")

dados3 <- tabela2 %>% select(ano,`Gols`) %>% arrange(ano)
...
objeto_0 <- dados3 %>%
  ...
  select(ano,Gols) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
titulo<-T_ST_P_No_Culturaesporte$TITULO[3]
subtexto<-"Fonte: Transfermarkt"
link <- T_ST_P_No_Culturaesporte$LINK[3]
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$Gols)),
                                   collapse = ' ')),']',sep = '')
...
write.csv2(exportJson0,file = paste('data/',gsub('.csv','',T_ST_P_No_Culturaesporte$NOME_ARQUIVO_JS[3]),
                               '.json',sep =''),fileEncoding = 'latin1')
write.csv2(texto,file = paste('data/',T_ST_P_No_Culturaesporte$NOME_ARQUIVO_JS[3],
